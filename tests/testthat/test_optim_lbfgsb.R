...
context("FunctionalModel.fit.lbfgsb")

test_that("Test FunctionalModel.fit.lbfgsb", {
  ...
  result <- FunctionalModel.fit.lbfgsb(metric, functionalModel, start);
  ...
  result <- FunctionalModel.fit.lbfgsb(metricr, functionalModel, startr);
  expect_identical(is.null(result), FALSE);
  expect_is(result, "FittedFunctionalModel");
