...
context("model.fit.nlslm")

test_that("Test model.fit.nlslm", {
  ...
  result <- model.fit.nlslm(metric, functionalModel, start);
  ...
  result <- model.fit.nlslm(metricr, functionalModel, startr);
  expect_identical(is.null(result), FALSE);
  expect_is(result, "FittedFunctionalModel");
