...
context("FunctionalModel.fit.pso")

test_that("Test FunctionalModel.fit.pso", {
  ...
  result <- FunctionalModel.fit.pso(metric, functionalModel, start);
  ...
  result <- FunctionalModel.fit.pso(metricr, functionalModel, startr);
  expect_identical(is.null(result), FALSE);
  expect_is(result, "FittedFunctionalModel");
