...
context("FunctionalModel.fit")

test_that("Test FunctionalModel.fit", {
  ...
  result <- FunctionalModel.fit(metric, functionalModel, start);
  ...
  result <- FunctionalModel.fit(metricr, functionalModel, startr);
  expect_identical(is.null(result), FALSE);
  expect_is(result, "FittedFunctionalModel");
