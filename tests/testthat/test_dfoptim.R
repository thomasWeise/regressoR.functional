...
context("model.fit.dfoptim")

test_that("Test model.fit.dfoptim", {
  ...
  result <- model.fit.dfoptim(metric, functionalModel, start);
  ...
  result <- model.fit.dfoptim(metricr, functionalModel, startr);
  expect_identical(is.null(result), FALSE);
  expect_is(result, "FittedFunctionalModel");
