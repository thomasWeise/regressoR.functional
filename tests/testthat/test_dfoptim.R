...
context("FunctionalModel.fit.dfoptim")

test_that("Test FunctionalModel.fit.dfoptim", {
  ...
  result <- FunctionalModel.fit.dfoptim(metric, functionalModel, start);
  ...
  result <- FunctionalModel.fit.dfoptim(metricr, functionalModel, startr);
  expect_identical(is.null(result), FALSE);
  expect_is(result, "FittedFunctionalModel");
