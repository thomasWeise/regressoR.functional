...
context("FunctionalModel.fit.cmaes")

test_that("Test FunctionalModel.fit.cmaes", {
  ...
  result <- FunctionalModel.fit.cmaes(metric, functionalModel, start);
  ...
  result <- FunctionalModel.fit.cmaes(metricr, functionalModel, startr);
  expect_identical(is.null(result), FALSE);
  expect_is(result, "FittedFunctionalModel");
