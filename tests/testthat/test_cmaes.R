...
context("model.fit.cmaes")

test_that("Test model.fit.cmaes", {
  ...
  result <- model.fit.cmaes(metric, functionalModel, start);
  ...
  result <- model.fit.cmaes(metricr, functionalModel, startr);
  expect_identical(is.null(result), FALSE);
  expect_is(result, "FittedFunctionalModel");
